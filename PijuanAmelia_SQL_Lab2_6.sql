...
SELECT last_name, count(last_name) as frequency FROM sakila.actor
GROUP BY last_name
HAVING frequency = 1;
...
SELECT staff_id, count(rental_id) as Number_of_rentals FROM sakila.rental
GROUP BY staff_id;

-- 4. Using the film table, find out how many films were released each year.

SELECT release_year, count(title) as movies_released  FROM sakila.film
GROUP BY release_year;

-- 5. Using the film table, find out for each rating how many films were there.

SELECT rating, count(title) as number_of_movies FROM sakila.film
GROUP BY rating;

-- 6. What is the mean length of the film for each rating type. Round off the average lengths to two decimal places

SELECT rating, sec_to_time(round(avg(length*60))) as movie_avg_duration FROM sakila.film
GROUP BY rating;
